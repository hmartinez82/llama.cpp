...
struct qnn_buf_s {
    ...
    std::condition_variable buffer_pool_cond_not_empty;
...
//  {.device = 0, .threads = 1, .name =   "qnn-cpu", .lib = "QnnCpu.dll", .instance = nullptr, .buffer_pool = nullptr, .backend = nullptr, .raw_interface = nullptr, .raw_system_interface = nullptr},
  {.device = 0, .threads = 1, .name =   "qnn-htp(aka dsp)", .lib = "QnnHtp.dll", .instance = nullptr, .buffer_pool = nullptr, .backend = nullptr, .raw_interface = nullptr, .raw_system_interface = nullptr},
};
...
static char * ggml_qnn_strndup(const char * source, size_t maxlen) {
    auto buf = malloc(maxlen);
    if (buf) {
        memscpy(buf, maxlen, source, maxlen);
    }
    return (char *)buf;
}
...
Fn load_qnn_functionpointers(HMODULE handle, const char * function_name) {
...
static void buffer_pool_free (buf_element_t * element) {
    ...
    self->buffer_pool_cond_not_empty.notify_one();

    self->buffer_pool_mutex.unlock();
}
...
static buf_element_t * buffer_pool_alloc (qnn_buf_t * self) {
    ...
    auto ul = std::unique_lock(self->buffer_pool_mutex);

    while (self->buffer_pool_num_free < 2) {
        self->buffer_pool_cond_not_empty.wait(ul);
    }
...
static void qnn_buf_buffer_clear (qnn_buf_t * fifo) {
    ...
    auto ul = std::unique_lock(fifo->mutex);
...
static int qnn_buf_buffer_size (qnn_buf_t * self) {
    ...
    auto ul = std::unique_lock(self->mutex);
...
private:
    ...
    static std::unordered_map<BackendIdType, HMODULE> _loaded_lib_handle;
...
std::unordered_map<qnn_instance::BackendIdType, HMODULE> qnn_instance::_loaded_lib_handle;
...
int qnn_instance::load_backend(std::string & lib_path, const QnnSaver_Config_t ** saver_config) {
    ...
    auto lib_handle = LoadLibrary(lib_path.c_str());
    if (nullptr == lib_handle) {
        QNN_LOG_WARN("can not open QNN library %s, with error: %s", lib_path.c_str(), GetLastError());
        return 1;
    }

    // load get_provider function
    auto get_providers = (_pfn_QnnInterface_getProviders*)(GetProcAddress(lib_handle, "QnnInterface_getProviders"));
    if (nullptr == get_providers) {
        QNN_LOG_WARN("can not load symbol QnnInterface_getProviders : %s", GetLastError());
        return 2;
    }
    ...
    if (_loaded_lib_handle.count(backend_id) > 0) {
        ...
        int dlclose_error = FreeLibrary(_loaded_lib_handle[backend_id]);
        if (dlclose_error != 0) {
            QNN_LOG_WARN("fail to close %p with error %s\n", _loaded_lib_handle[backend_id], GetLastError());
        }
    }
...
int qnn_instance::unload_backend() {
    ...
    for (auto &it : _loaded_lib_handle) {
        dlclose_error = FreeLibrary(it.second);
        if (dlclose_error != 0) {
...
int qnn_instance::load_system() {
    ...
    auto get_providers = (_pfn_QnnSystemInterface_getProviders*)(GetProcAddress(_system_lib_handle, "QnnSystemInterface_getProviders"));
    if (nullptr == get_providers) {
...
int qnn_instance::unload_system() {
    ...
    int dlclose_error = FreeLibrary(_system_lib_handle);
    if (dlclose_error != 0) {
...
static void ggml_qnn_add(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    Qnn_Param_t qnn_params[]                    = { QNN_PARAMTYPE_UNDEFINED };
    ...
    if (!graph_initialized) {
        ...
        Qnn_OpConfig_t opconfig = {
                .version = (Qnn_OpConfigVersion_t) 1,
                .v1 = {
                        "ggml_op_add",
...
static void * ggml_qnn_host_malloc(size_t n) {
    ...
    void * data = _aligned_malloc(n, si.dwPageSize);

    if (data == nullptr) {
        QNN_LOG_WARN("%s: error: _aligned_malloc failed\n", __func__);
    }
...
static ggml_backend_buffer_t ggml_backend_qnn_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    const size_t size_page = si.dwPageSize;
...
struct ggml_compute_state_shared {
    ...
    std::atomic<int> n_active;  // num active threads
    std::atomic<int> node_n;    // active graph node
    std::atomic<int> node_task; // active graph node task phase
...
ggml_backend_buffer_type_t ggml_backend_qnn_buffer_type(size_t device_index) {
    ...
    static struct ggml_backend_buffer_type ggml_backend_buffer_types_qnn[GGML_QNN_MAX_DEVICES];
